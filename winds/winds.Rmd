...
- un champ `polygones` contenant les contours des 12 régions monéatires des USA 
```

```{block, box.title = "Calculez les balances", box.body = list(fill = "white"), box.icon = "fa-star"}
Calculez les balances en chaque noeuds de la matrice OD et stockez le résultats dans une colonne delta de la data.frame polygones.  
```
...
Importez les données `FD_MIGCOM_2018AGG.rds` à l'aide de la fonction `readRDS`. Cette data.frame correspond aux données de détails sur les migrations résidentielles française de l'INSEE  [(https://www.insee.fr/fr/statistiques/5395755?sommaire=5395764#dictionnaire)](https://www.insee.fr/fr/statistiques/5395755?sommaire=5395764#dictionnaire). Cette base de données à juste été aggrégée pour ne garder qu'un facteur de variation la catégorie socio-professionelle `CSM` de manière à la rendre un peu plus légère.
```
...
Dans cette base, la commune de destination est renseignée avec les codes des arrondissements municipaux pour les villes ayant plusieurs arrondissement. Nous allons commncer par créer une table ou les mouvements sont des mouvements de communes à communes. Pour ce faire, créez à partir de la table précédente une table `com_arr` contenant pour chaque communes ayant des arrondissements le code d'arrondissement (colonne `ARM`) et le code commune (colonne `COMMUNE`). Vous devriez obtenir la table suivante :
```
```{r, echo=FALSE}
...
- COMMUNE : commune de résidance
- COMMUNE_ANM1 : commune de résidance (année n-1)
- CSM : catégorie socio professionelle
- IPONDI : poid de la ligne
- DEP : département de résidance 
...
Filtrez la table pour ne cnserver que :
...
Calculez pour chaque comunes en utilisant la table précédente :
...
Réalisez une carte symbole proportionelle des balances, la taille des symboles encodera la valeur abolue de la balance et la couleur le signe. Réalisez ensuite une carte en symboles +/-.
```

```{r, echo=FALSE}
delta_dep = delta_geo %>% group_by(INSEE_DEP) %>% summarize(delta=sum(delta)) %>% ungroup() %>% select(delta)
library(ggplot2)
ggplot(delta_dep)+geom_sf() + geom_sf(data=delta_dep %>% st_centroid(),aes(size=abs(delta),color=delta))+scale_color_gradient2()
```
...
```{block, box.title = "Calcul du champ potentiel", box.body = list(fill = "white"), box.icon = "fa-star"}
Représentez le champ potentiel et le champ véctoriel.
```
